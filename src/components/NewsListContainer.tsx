...
import { fetchNewsByDate, addVisibleNews, addLoadCount } from '../redux/slice/NewsSlice';
import { store } from '../redux/store';
import NewsList from './NewsList';

export default function NewsListContainer() {
  const dispatch = useDispatch<AppDispatch>();
  const { loading, error, visibleItems } = useSelector((state: RootState) => state.news);


  const loadNewItems = ()=>{
    const currentLoadCount = store.getState().news.loadCount; // 최신 loadCount 상태 가져오기
    const items = store.getState().news.items;
    const newItems = items.slice(currentLoadCount * 10, (currentLoadCount + 1) * 10);
